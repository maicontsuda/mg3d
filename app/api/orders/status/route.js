...
async function sendEmail(order, message) {
  if (!process.env.RESEND_API_KEY || !process.env.RESEND_FROM_EMAIL || !order.customers?.email) return { channel: 'email', sent: false, reason: 'provider_not_configured_or_email_missing' }
  try {
    const response = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${process.env.RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: process.env.RESEND_FROM_EMAIL, to: [order.customers.email], subject: `MG3D — atualização do pedido #${order.id}`, text: message }) })
    return { channel: 'email', sent: response.ok, reason: response.ok ? undefined : 'provider_rejected_request' }
  } catch { return { channel: 'email', sent: false, reason: 'provider_unreachable' } }
}

async function sendWhatsApp(order, message) {
  if (!process.env.TWILIO_ACCOUNT_SID || !process.env.TWILIO_AUTH_TOKEN || !process.env.TWILIO_WHATSAPP_FROM || !order.customers?.phone) return { channel: 'whatsapp', sent: false, reason: 'provider_not_configured_or_phone_missing' }
  try {
    const phone = order.customers.phone.trim().replace(/^(whatsapp:)+/i, '')
    const params = new URLSearchParams({ From: `whatsapp:${process.env.TWILIO_WHATSAPP_FROM}`, To: `whatsapp:${phone}`, Body: message })
  ...
  } catch { return { channel: 'whatsapp', sent: false, reason: 'provider_unreachable' } }
}
...
export async function POST(request) {
  ...
  if (userError || user?.email?.toLowerCase() !== 'maicntsuda@gmail.com') return Response.json({ error: 'Acesso administrativo não autorizado.' }, { status: 403 })

  const body = await request.json().catch(() => ({}))
  const id = Number(body.id)
  const status = body.status
  if (!Number.isInteger(id) || !Object.hasOwn(statusLabels, status)) return Response.json({ error: 'Pedido ou status inválido.' }, { status: 400 })

  const { data: previous, error: previousError } = await supabase.from('orders').select('status').eq('id', id).single()
  if (previousError) return Response.json({ error: previousError.message }, { status: 404 })
  if (previous.status === status) return Response.json({ order: previous, notifications: [], unchanged: true })
